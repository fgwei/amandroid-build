...
*/
/**
 * From https://github.com/ijuma/junit_xml_listener
 */
...
class JUnitXmlTestsListener(val outputDir : String) extends TestsListener {
  ...
  class TestSuite(val name : String) {
    val events : ListBuffer[sbt.testing.Event] = new ListBuffer()
    val start = System.currentTimeMillis
    var end = System.currentTimeMillis

    /**Adds one test result to this suite.*/
    def addEvent(e : sbt.testing.Event) = events += e
    ...
    def count() : (Int, Int, Int) = {
      ...
      for (e <- events) {
        e.status match {
          case sbt.testing.Status.Error   => errors += 1
          case sbt.testing.Status.Failure => failures += 1
          case _                          =>
        }
      }
                       ...
                       for (e <- events) yield <testcase classname={ name } name={ e.fullyQualifiedName } time={ "0.0" }>
                                                 {
                                                   ...
                                                   e.status match {
                                                     case sbt.testing.Status.Error if (e.throwable.isDefined) => <error message={ e.throwable.get.getMessage } type={ e.throwable.get.getClass.getName }>{ trace }</error>
                                                     case sbt.testing.Status.Error => <error message={ "No Exception or message provided" }/>
                                                     case sbt.testing.Status.Failure if (e.throwable.isDefined) => <failure message={ e.throwable.get.getMessage } type={ e.throwable.get.getClass.getName }>{ trace }</failure>
                                                     case sbt.testing.Status.Failure => <failure message={ "No Exception or message provided" }/>
                                                     case sbt.testing.Status.Skipped => <skipped/>
                                                     case _ => {}
                                                   }
                                                 }
                                               </testcase>
