// Modified to allow zip/jar to not set time for entries, Robby <robby@ksu.edu>
...
object MyIO
{
  ...
  * @param manifest The manifest for the jar.*/
  def jar(sources: Traversable[(File,String)], outputJar: File, manifest: Manifest, setTime : Boolean = true): Unit =
    archive(sources.toSeq, outputJar, Some(manifest), setTime)
  /** Creates a zip file.
  * @param sources The files to include in the zip file paired with the entry name in the zip.
  * @param outputZip The file to write the zip to.*/
  def zip(sources: Traversable[(File,String)], outputZip: File, setTime : Boolean = true): Unit =
    archive(sources.toSeq, outputZip, None, setTime)
  ...
  private def archive(sources: Seq[(File,String)], outputFile: File, manifest: Option[Manifest], setTime : Boolean = true)
  {
    ...
    else
    {
      ...
      { output =>
        ...
        writeZip(sources, output, setTime)(createEntry)
      }
    }
  }
  private def writeZip(sources: Seq[(File,String)], output: ZipOutputStream, setTime : Boolean)(createEntry: String => ZipEntry)
  {
...
//      log.debug("\tAdding directory " + relativePath + " ...")
      ...
      e setTime (if (setTime) now else 0)
      e setSize 0
...
//      log.debug("\tAdding " + file + " as " + name + " ...")
      val e = createEntry(name)
      e setTime (if (setTime) file.lastModified else 0)
      e
    }
